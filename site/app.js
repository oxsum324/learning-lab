import {TOPICS} from './topics.js?v=0.9.0';
import {KEY,DAY_KEY,TOPIC_ID,SANDBOX,emptyState,assess,validateState,mergeStates,safeSave} from './core.js?v=0.9.0';
import {DrawingEditor,renderDrawing} from './drawing.js?v=0.9.0';
import {SHEETS,hasDrawing} from './drawing-model.js?v=0.9.0';
import {GUIDES} from './teaching-guides.js?v=0.9.0';
...
$('export').onclick=()=>{pause();const data={...state,exportedAt:now(),source:'learning-lab-browser-v0.9.0'};download(JSON.stringify(data,null,2),`learning-lab-${TOPIC_ID}-${new Date().toISOString().slice(0,10)}.json`);$('export-status').textContent='已交給瀏覽器下載，請確認檔案確實保存。紀錄尚未同步雲端。';};
...
$('topic-select').onchange=e=>{const next=e.target.value;if(course){pause();capture();if(!persist()){$('topic-select').value=TOPIC_ID;storageMessage('尚有未保存資料，請先匯出備份，再切換主題。',true);return;}}const url=new URL(location.href);url.searchParams.set('topic',next);url.searchParams.set('v','0.9.0');url.hash='';location.assign(url.href);};
