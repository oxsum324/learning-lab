import {SHEETS,COLORS,emptyDrawing,validDrawing,shape,moveShape,makeTemplate} from './drawing-model.js?v=0.9.0';
...
export class DrawingEditor{
 ...
 mount(){this.root.innerHTML=`<div class="drawing-head"><h3>圖解作答</h3><span class="active-tool" aria-live="polite"></span><div class="drawing-history"><button type="button" data-action="undo">復原</button><button type="button" data-action="redo">重做</button></div></div>
 <div class="drawing-sheets" role="group" aria-label="圖紙種類"></div>
 ...
 <details class="drawing-menu"><summary>底圖／編輯</summary><div class="drawing-actions"><label>底圖<select data-prop="template"><option value="simple">簡支梁</option><option value="cantilever">懸臂梁</option><option value="propped">支承懸臂</option><option value="axes">剪力／彎矩基線</option></select></label><button type="button" data-action="template">加入底圖</button><button type="button" data-action="stamp">置於中央</button><button type="button" data-action="cut">剪下</button><button type="button" data-action="copy">複製</button><button type="button" data-action="paste">貼上</button><button type="button" data-action="reverse">反向</button><button type="button" data-action="rotate">旋轉支承90°</button><button type="button" data-action="delete">刪除選取</button><button type="button" data-action="export">下載此圖 SVG</button></div><label>選取已畫圖形<select data-prop="selection" aria-label="選取已畫圖形"></select></label></details>
 </div>
 <div class="drawing-board"></div>
 <p class="drawing-status" role="status">拖曳畫圖；在畫布外滑動可捲頁。</p>
 <details class="drawing-help"><summary>操作說明與保存方式</summary><p class="muted">選工具後拖曳畫線、箭頭或曲線，點一下放支承或文字。選取圖形可拖移或用方向鍵微調；拖曳藍色端點可改長度。底圖只含構件與支承。手機橫放可增加畫圖空間。</p><p class="muted">圖解隨草稿與 JSON 備份保存。可以只畫圖提交，由教練審閱；系統不會自動判定受力圖或變形圖正確。</p></details>`;
 ...
 for(const k of ['select','pen','arrow','line','udl','moment','pin','roller','fixed','text','curve']){const n=toolNames[k];const b=makeButton(n,()=>{this.cancelDrag();this.tool=k;this.closeMenus();if(['text','curve'].includes(k))this.root.querySelector('.drawing-settings').open=true;this.paint();if(k==='text')this.prop('text').focus();else this.board.focus({preventScroll:true});});b.dataset.tool=k;b.setAttribute('aria-label',n);b.title=n;b.textContent=({select:'選取',line:'直線',arrow:'箭頭'})[k]??n;this.root.querySelector(['select','pen','arrow','line'].includes(k)?'.drawing-tools':'.drawing-extra-tools').append(b);}
 ...
 up(){if(!this.drag)return;const d=this.drag;this.drag=null;const s=this.items()[d.index];if(d.newShape&&['line','arrow','udl','curve'].includes(s.type)&&Math.hypot(s.x2-s.x1,s.y2-s.y1)<3)s.x2=Math.min(1000,s.x1+150);this.save(d.before);this.message(`${SHEETS[this.key]}：${this.items().length}個物件。個別圖紙自動存草稿。`);}
