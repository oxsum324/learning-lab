import {TOPICS} from './topics.js?v=0.11.0';
import {KEY,DAY_KEY,TOPIC_ID,SANDBOX,emptyState,assess,validateState,mergeStates,safeSave} from './core.js?v=0.11.0';
import {DrawingEditor,renderAnswerDrawing} from './drawing.js?v=0.11.0';
import {hasDrawing} from './drawing-model.js?v=0.11.0';
import {GUIDES} from './teaching-guides.js?v=0.11.0';
import {mountCloudHistory,mergeCloudRecords} from './cloud-history.js?v=0.11.0';
let cloud;
...
if(SANDBOX){document.querySelector('.notice').replaceChildren('功能測試空間：測試紀錄與正式學習紀錄分開儲存。',$('cloud-indicator'));document.querySelector('.version').textContent+=' · 功能測試';}
...
try{expected=localStorage.getItem(KEY);if(expected)state=validateState(JSON.parse(expected));storageMessage('目前瀏覽器可用；雲端狀態另列於下方。');}catch{writable=false;storageMessage('無法讀取原有儲存資料，已保留原資料並停止覆寫。新紀錄請立即匯出，勿清除瀏覽器資料。',true);}
function persist(){if(!writable)return false;try{expected=safeSave(localStorage,state,expected);storageMessage('已存至目前瀏覽器 · '+new Date().toLocaleTimeString('zh-TW')+'');cloud?.changed();return true;}catch(e){writable=false;storageMessage(e.message+' 儲存未完成，請先匯出本頁資料。',true);return false;}}
...
$('export').onclick=()=>{pause();const data={...state,exportedAt:now(),source:'learning-lab-browser-v0.11.0'};download(JSON.stringify(data,null,2),`learning-lab-${TOPIC_ID}-${new Date().toISOString().slice(0,10)}.json`);$('export-status').textContent='已交給瀏覽器下載，請確認檔案確實保存。雲端保存狀態請見雲端歷史區。';};
...
$('topic-select').onchange=e=>{const next=e.target.value;if(course){pause();capture();if(!persist()){$('topic-select').value=TOPIC_ID;storageMessage('尚有未保存資料，請先匯出備份，再切換主題。',true);return;}}const url=new URL(location.href);url.searchParams.set('topic',next);url.searchParams.set('v','0.11.0');url.hash='';location.assign(url.href);};
if(TOPIC_ID!=='structural-analysis'){
 ...
 $('problem-note').textContent='本頁列出短題時直接依題設作答；標示自選教材或官方原卷時，請記錄題源與完整條件。係數依題設模型；條文查核需註明規範版本。完整作答由教練審閱。';
...
cloud=mountCloudHistory({topicId:TOPIC_ID,sandbox:SANDBOX,getState:()=>state,download,onMerge:remote=>{
 if(!course)throw Error('請等待教材載入後再合併。');
 if(!writable)throw Error('本頁無法安全寫入，請先匯出紀錄並重新整理。');
 pause();const merged=mergeCloudRecords(state,remote);
 const next=safeSave(localStorage,merged,expected);state=merged;expected=next;showDay(currentDay);paintStats();
}});
